...
export default class GridFilter extends Component {
    constructor(props) {
        ...
        this.state = {
            toggleTooltip: false,
            toolTipId: '',
            columnFilter: this.props.columnHeaders[0], //if no value is selected, default value will be first element of array
        ...
        return (
            <div className="table-filter-main-section">
                <div className="table-filter-sub-section">
                    <label className="table-filter-label" htmlFor="columnFilter">Column</label>
                    <select
                        ...
                        value={this.state.columnFilter}>
                        {
                            this.props.columnHeaders.map((data, i) =>
                                <option value={data.field} key={i}>{data.headerName}</option>
                            )
                        }
                    </select>
                </div>
                <div className="table-filter-sub-section">
                    <label className="table-filter-label" htmlFor="operatorFilter">Operator</label>
                    <select
                        className="table-filter-dropdown"
                        id="operatorFilter"
                        onChange={(e) => this.handleFilterItems(e, "dropdown")}
                        ...
                        {
                            filterOperatorArray && filterOperatorArray.map((data, i) =>
                ...
                <div className="table-filter-sub-section">
                    <label className="table-filter-label" htmlFor="valueFilter">Value</label>
                    <input
                        className="table-filter-dropdown"
                        id="valueFilter"
                ...
                <div className="table-filter-sub-section">
                    <button className="table-filter-close-btn"
                        onClick={() => this.props.handleApplyFilter(this.state.columnFilter, this.state.operatorFilter, this.state.valueFilter)}>
